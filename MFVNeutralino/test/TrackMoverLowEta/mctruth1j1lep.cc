...
int main(int argc, char** argv) {
  ...
  nr.init_options("mfvMovedTreeMCTruth/t", "TrackMoverMCTruth_LowEta", "trackmovermctruthulv13lepmv6", "all_signal = True")
  ...
  enum { k_decay_x, k_decay_y, k_decay_z, k_decay_xy, k_lspdist2, k_lspdphi, k_lspdeta, k_lspdr, k_lspasymdecay, k_lspcostheta,  k_lspdist3, 
         ...
         k_pt0, k_fullpt0, k_mupt1, k_elept1, k_ntks_j0, k_jet_dr_minj0_q0, k_mu_dr_minl1_l1, k_ele_dr_minl1_l1, k_boost0_boost1, 
         ...
         k_mu1_p, k_ele1_p, k_jet0_sump_mu1_p, k_jet0_sump_ele1_p, k_jet0_sump_lep1_p,
  ...
  for (numdens& nd : nds) {
    ...
    nd.book(k_fullpt0, "fullpt0", "; full RECO jet0 pT [GeV]", 100, 0, 500);
  ...
  auto fcn = [&]() {
    ...
    float   jet_pt_0 = -9.9, fulljet_pt_0 = -9.9;
    ...
    for (int ilsp = 0; ilsp < 2; ++ilsp) {
      ...
      if (semilep) { 
        ...
        fulljet_p4_0.SetPxPyPzE(0,0,0,0);
        ...
        for (int j = 0; j < tks.n(); ++j) {
          ...
          if ( jit0 != jet0_tracks.end()){ 
            sumpreco_0 += tks.p(j);
            fulljet_p4_0 += tks.p4(j); 
          }

        }  
        // std::cout << "full jet pT : " << fulljet_p4_0.Pt() << std::endl;
        for (int j = 0; j < tks.n(); ++j) {
          const TLorentzVector jp4 = tks.p4(j);
          auto jit0 = std::find(jet0_tracks.begin(), jet0_tracks.end(), j);
