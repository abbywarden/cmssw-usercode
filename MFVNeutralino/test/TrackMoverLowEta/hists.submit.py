          ...
          if apply_correction:
            for tm in ["sim","dat"] :
              ...
              if tm == "sim":
                ...
                w_fn_2d_move = "reweight_v2p5_loweta_move_sim_vetodr_tau%06ium_M%02i_%s_2D.root" % (tau, mg, year) 
              else:
                ...
                w_fn_2d_move = "reweight_v2p5_loweta_move_dat_vetodr_tau%06ium_M%02i_%s_2D.root" % (tau, mg, year) 
              ...
              batch = 'TrackMover_StudyV2p5_LowEta_NoPreSelRelaxBSPNotwVetodR0p4JetByJetHists' + version.capitalize() + '_%i%i_tau%06ium_M%02i_%s' % (nl, nb, tau, mg, batch_tag)
            ...
            batch = 'TrackMover_StudyV2p5_LowEta_NoPreSelRelaxBSPNotwVetodR0p4JetByJetHists' + version.capitalize() + '_%i%i_tau%06ium_%s' % (nl, nb, tau, batch_tag)
            args = '-t mfvMovedTree%i%i --tau %i %s' % (nl, nb, tau, correction_args) 
