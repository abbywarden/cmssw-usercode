...
void MakeWeightPlots(bool Is_bkg, const char* boson, int mg, int ctau, const char* etabin, const char* year)
{
  ...
  *it = (char) tolower(*it);
  ...
  if (Is_bkg)
     fnout.Form("~/nobackup/crabdirs/TM_2D_kin_weight_sim_lepton_histos/reweight_v2p4_alletadown_kin_sim_vetodr_tau%06ium_M%02i_2D.root", ctau, mg);
  else
     fnout.Form("~/nobackup/crabdirs/TM_2D_kin_weight_dat_lepton_histos/reweight_v2p4_alletadown_kin_dat_vetodr_tau%06ium_M%02i_2D.root", ctau, mg);
