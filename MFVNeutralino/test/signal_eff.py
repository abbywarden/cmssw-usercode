...
ps = plot_saver(plot_dir('nrequest_%s' % version), size=(600,600), pdf=True, log=False)
...
for sample in stoplb + stopld:
    ...
    fn = os.path.join('/nfs_scratch/acwarden/Lepton_Histos_MiniTrees/MiniTree%s' % version, sample.name + '.root')    
...
per = PerSignal('efficiency', y_range=(0.,200000))
...
#ps.save('sigeff_loose')
ps.save('nrequest_1loose')
