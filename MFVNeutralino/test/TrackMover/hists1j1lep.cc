...
int main(int argc, char** argv) {
  ...
  //   // "nocuts_jet0_sump_jetmudr_den",
  //   // "nocuts_jet0_sump_jeteledr_den",
  //   "nocuts_jet0_sumpt_jetmudr_den",
  //   "nocuts_jet0_sumpt_jeteledr_den",
  ...
  //trial 1 order : jet-lep dR, jet sump, lepton pT 
  //trial 2 order : jet sump, lepton pT, jet-lep dR 

  const std::vector<std::string> jetlep_1d_kin_weights_hists = {
    // "nocuts_jet0_sump_den",
    "nocuts_jet0_sumpt_den",
    // "nocuts_pt0_den",
    // "nocuts_elept1_den",
    // "nocuts_mupt1_den",
    "nocuts_jetele_dr_den", //use w/ sump(t)
    "nocuts_jetmu_dr_den", //use w/ sump(t)
    // "nocuts_fulljetele_dr_den", //use w/ pt0
    // "nocuts_fulljetmu_dr_den", //use w/ pt0
  };
  ...
  if (jetlep_kin_weights) std::cout << "also using 1D weights " << std::endl;
  ...
  enum { k_movedist2, k_movedist3, k_movevectoreta, k_npv, k_pvx, k_pvy, k_pvz, k_pvrho, k_pvntracks, k_pvscore, 
         ...
         k_jetele_asymm, k_jet0_eta, k_mu1_eta, k_ele1_eta, k_jetmu_dr, k_jetele_dr, k_fulljetmu_dr, k_fulljetele_dr, k_jetmu_costheta, k_jetmu_deta,
         ...
         k_jet0_trk_pt, k_mu1_trk_pt, k_ele1_trk_pt, k_jet0_trk_p, k_mu1_p, k_ele1_p, k_jet0_sump, k_jet0_sumpt, k_mu1_p_eta, k_ele1_p_eta, k_lep1_p_eta,
         k_jet0_maxeta_mu1_eta, k_jet0_sump_mu1_p, k_jet0_maxeta_ele1_eta, k_jet0_sump_ele1_p, k_jet0_sump_lep1_p, k_jet0_sump_ntks, 
  ...
  for (numdens& nd : nds) {
    ...
    nd.book(k_fulljetmu_dr, "fulljetmu_dr", ";jet-mu #DeltaR; arb. units", 60, 0, 6); //check ##
    nd.book(k_fulljetele_dr, "fulljetele_dr", ";jet-ele #DeltaR; arb. units", 60, 0, 6); //check##
    ...
    nd.book(k_mupt1, "mupt1", ";RECO mu1 pT [GeV]", 50, 0, 500);//check ##
    nd.book(k_elept1, "elept1", ";RECO ele1 pT [GeV]", 50, 0, 500); //check##
    ...
    nd.book(k_jetlepdeltadz, "jetlepdeltadz", ";jet-lep delta dz;events", 100, 0, 5.0);
  ...
  auto fcn = [&]() {
    ...
    const float jetlepdeltadz = nt.tm().jetlepdeltadz();
    ...
    //there is a new requirement that the jet is moved only if it is within 2cm of the lepton in dz. therefore, now need an if statement to protect cases when no jet is moved. 
    ...
    if (jj != -1) { 
      ...
      auto p4 = fulljet0_p4 = jets.p4(jj);
      jet0_p = fulljet0_p4.P();
    ...
    if (lep_ismu > 0) {
      ...
      // std::cout << "muons are med? pass iso? eta? hltmatched? passtrigpt? " << muons.isMed(ll) << " " << muons.iso(ll) << " " << fabs(muons.eta(ll))  << " "<< muons.hltmatched(ll) << " " << muons.passtrigpt(ll) << std::endl;
      if (muons.isMed(ll) && muons.iso(ll) < 0.1 && fabs(muons.eta(ll)) < 2.4 &&  muons.pt(ll) >= 27.0) //muons.hltmatched(ll) && muons.passtrigpt(ll))
    ...
    if (lep_isele > 0) { 
      ...
      if (electrons.isTight(ll) && fabs(electrons.eta(ll)) < 2.4 && electrons.pt(ll) >= 35.0) //electrons.hltmatched(ll) && electrons.passtrigpt(ll))
    ...
    if (lep_isele > 0) { 
      jetele_asymm = (fulljet0_p4.Pt() - ele1_p4.Pt()) / (fulljet0_p4.Pt() + ele1_p4.Pt());
      jetele_dr = jet0_p4.DeltaR(ele1_p4); 
      fulljetele_dr = fulljet0_p4.DeltaR(ele1_p4); 
      jetele_costheta = ((fulljet0_p4.X()*ele1_p4.X()) + (fulljet0_p4.Y()*ele1_p4.Y()) + (fulljet0_p4.Z()*ele1_p4.Z()))/(fulljet0_p4.P()*ele1_p4.P()); 
      jetele_dphi = fulljet0_p4.DeltaPhi(ele1_p4); 
      jetele_deta = fabs(fulljet0_p4.Eta() - ele1_p4.Eta()); 
      // const double jetele_dind = fabs(jetele_i[1] - jetele_i[0]);
      jetele_mv_dphi_sum = move_vector.DeltaPhi((fulljet0_p4 + ele1_p4).Vect());
      jetele_mv_deta_sum = fabs((fulljet0_p4 + ele1_p4).Eta() - move_vector.Eta());
    }
    if (lep_ismu > 0) { 
      jetmu_asymm = (fulljet0_p4.Pt() - mu1_p4.Pt()) / (fulljet0_p4.Pt() + mu1_p4.Pt());

      jetmu_dr = jet0_p4.DeltaR(mu1_p4); 
      fulljetmu_dr = fulljet0_p4.DeltaR(mu1_p4); 

      jetmu_costheta = ((fulljet0_p4.X()*mu1_p4.X()) + (fulljet0_p4.Y()*mu1_p4.Y()) + (fulljet0_p4.Z()*mu1_p4.Z()))/(fulljet0_p4.P()*mu1_p4.P()); 
      jetmu_dphi = fulljet0_p4.DeltaPhi(mu1_p4); 
      jetmu_deta = fabs(fulljet0_p4.Eta() - mu1_p4.Eta()); 
      // const double jetmu_dind = fabs(jetmu_i[1] - jetmu_i[0]);
      jetmu_mv_dphi_sum = move_vector.DeltaPhi((fulljet0_p4 + mu1_p4).Vect());
      jetmu_mv_deta_sum = fabs((fulljet0_p4 + mu1_p4).Eta() - move_vector.Eta());
    ...
    if (jetlep_kin_weights){
      ...
      for (const auto& name : jetlep_1d_kin_weights_hists) {
        ...
        if (name == "nocuts_jet0_sumpt_den")
          v = sumpt_0;

        // else if (name == "nocuts_mupt1_den" && lep_ismu)
        //   v = mu1_pt;
        // else if (name == "nocuts_elept1_den" && lep_isele)
        //   v = ele1_pt;

        else if (name == "nocuts_jetmu_dr_den" && lep_ismu)
          v = jetmu_dr;
        else if (name == "nocuts_jetele_dr_den" && lep_isele)
          v = jetele_dr;

        // else if (name == "nocuts_fulljetmu_dr_den" && lep_ismu)
        //   v = fulljetmu_dr;
        // else if (name == "nocuts_fulljetele_dr_den" && lep_isele)
        //   v = fulljetele_dr;
    ...
    if (jet_decay_weights) {
      ...
      //   //trial 1
      //   // if (name == "nocuts_jet0_sump_jeteledr_den"){
      //   //   if (lep_isele) {
      //   //     vx = sump_0;
      //   //     vy = jetele_dr;
      //   //   }
      //   // }
      //   // else if (name == "nocuts_jet0_sump_jetmudr_den"){
      //   //   if (lep_ismu)  {
      //   //     vx = sump_0;
      //   //     vy = jetmu_dr;
      //   //   }
      ...
      //   if (name == "nocuts_jet0_sumpt_jeteledr_den"){
      //     if (lep_isele) {
      //       vx = sumpt_0;
      //       vy = jetele_dr;
      //     }
      //   }
      //   else if (name == "nocuts_jet0_sumpt_jetmudr_den"){
      //     if (lep_ismu)  {
      //       vx = sumpt_0;
      //       vy = jetmu_dr;
      //     }
  ...
  //  }
    ...
    //finally make certain that the moved muon passes offline cuts 
    //tighter selections : if there is a reco lepton, it must pass offline lepton selections (pt, eta, ID, iso, hltmatched)
    if ( (mu1_p > 0 && !mu1_pass_offline) || (ele1_p > 0 && !ele1_pass_offline))
      NR_loop_cont(w);
    ...
    for (numdens& nd : nds) {
      ...
      nd.den(k_jet0_sump_ntks, sump_0, jet_ntk_0);
      ...
      if (lep_ismu > 0) {
        ...
        nd.den(k_fulljetmu_dr, fulljetmu_dr); 
        ...
        nd.den(k_mu1_dxybs, mu1_dxybs);
        nd.den(k_jetpt0_jmasymm, fulljet0_p4.Pt(), jetmu_asymm);
        nd.den(k_mupt1_asymm, mu1_p4.Pt(), jetmu_asymm);
        nd.den(k_jeteta0_jmasymm, fulljet0_p4.Eta(), jetmu_asymm);
      ...
      if (lep_isele > 0) {
        ...
        nd.den(k_fulljetele_dr, fulljetele_dr); 
        ...
        nd.den(k_ele1_dxybs, ele1_dxybs);
        nd.den(k_jetpt0_jeasymm, fulljet0_p4.Pt(), jetele_asymm);
        nd.den(k_elept1_asymm, ele1_p4.Pt(), jetele_asymm);
        nd.den(k_jeteta0_jeasymm, fulljet0_p4.Eta(), jetele_asymm);
      ...
      nd.den(k_jetlepdeltadz, jetlepdeltadz);
    ...
    for (int i = 0; i < num_numdens; ++i) {
      ...
      nd.num(k_jet0_sump_ntks, sump_0, jet_ntk_0);
      ...
      if (lep_ismu > 0) {
        ...
        nd.num(k_fulljetmu_dr, fulljetmu_dr);
        ...
        nd.num(k_mu1_dxybs, mu1_dxybs);
        nd.num(k_jetpt0_jmasymm, fulljet0_p4.Pt(), jetmu_asymm);
        nd.num(k_mupt1_asymm, mu1_p4.Pt(), jetmu_asymm);
        nd.num(k_jeteta0_jmasymm, fulljet0_p4.Eta(), jetmu_asymm);
      ...
      if (lep_isele > 0) {
        ...
        nd.num(k_fulljetele_dr, fulljetele_dr);
        ...
        nd.num(k_ele1_dxybs, ele1_dxybs);
        nd.num(k_jetpt0_jeasymm, fulljet0_p4.Pt(), jetele_asymm);
        nd.num(k_elept1_asymm, ele1_p4.Pt(), jetele_asymm);
        nd.num(k_jeteta0_jeasymm, fulljet0_p4.Eta(), jetele_asymm);
      ...
      nd.num(k_jetlepdeltadz, jetlepdeltadz);
