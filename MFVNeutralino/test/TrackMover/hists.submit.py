...
for nl in 1,: # 3
    for nb in 0,: # 1, 2: #CHANGE FOR BJET
      for tau in [10000] : #[100, 300,1000, 3000, 30000] :
        for mg in [1000] : #[15,40,55,]:
          if apply_correction:
            #for tm in ["sim","dat"] :
            for tm in ["sim"]:
              w_fn_2d_move = ""
              if tm == "sim":
                ...
                w_fn_2d_move = "reweight_move_sim_vetodr_tau%06ium_M%04i_%s_2D.root" % (tau, mg, year) 
              else:
                ...
                w_fn_2d_move = "reweight_move_dat_vetodr_tau%06ium_M%04i_%s_2D.root" % (tau, mg, year) 
              batch_tag = "2DCorrection_jetsumptjetlepdr"
              ...
              #using 1d 
              w_fn_1d_kin = "reweight_stopld_tau%06ium_M%04i_1D_%s.root" % (tau, mg, year)
              # w_fn_1d_kin = "reweight_rebin_stopld_tau%06ium_M%04i_1D_%s.root" % (tau, mg, year)
              # w_fn_1d_kin = "reweight_ttbar_stopld_tau%06ium_M%04i_1D_%s.root" % (tau, mg, year)
              ...
              correction_args = '--jetlep-kinweights true --w_fn_1d_kin "%s" --jet-decayweights true --w_fn_2d_move "%s" --tm "%s"' % (w_fn_1d_kin, w_fn_2d_move, tm)

              # w_fns = [w_fn_1d_kin, w_fn_2d_kin, w_fn_2d_move]
              w_fns = [w_fn_1d_kin, w_fn_2d_move]
              #w_fns = [w_fn_2d_kin, w_fn_2d_move]


              batch = 'TrackMover_' + version.capitalize() + '_%i%i_tau%06ium_M%02i_%s' % (nl, nb, tau, mg, batch_tag)
            ...
            batch = 'TrackMover_' + version.capitalize() + '_%i%i_%s' % (nl, nb, batch_tag)
