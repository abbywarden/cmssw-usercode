        ...
        for mg in [55, ] : #[15,40,55,]:
          ...
          if apply_correction:
            for tm in ["sim",] :
              ...
              batch_tag = "2Dmovedist3movedist2jetdrllpsumpCorrection"
              #batch_tag = "2Dmovedist3movedist2jetdrllpsumpjet0etaCorrection"
              #batch_tag = "2Dmovedist3movedist2jetdrjet0sumpjet1sumpCorrection"
              w_fn_2d_kin = "reweight_v2_b2b_kin_vetodr_tau%06ium_M%02i_%i_2D.root" % (tau, mg, year) 
              w_fn_2d_move = "reweight_v2_b2b_move_vetodr_tau%06ium_M%02i_%i_2D.root" % (tau, mg, year) 
              w_fn_1d_kin = "reweight_v2_b2b_kin_vetodr_tau%06ium_M%02i_%i_1D.root" % (tau, mg, year) 
              ...
              batch = 'TrackMover_StudyV2_B2BV2_NoPreSelRelaxBSPNotwVetodR0p4JetByJetHists' + version.capitalize() + '_%i%i_tau%06ium_M%02i_%s' % (nl, nb, tau, mg, batch_tag)
            ...
            batch = 'TrackMover_StudyV2_NoPreSelRelaxBSPNotwVetodR0p4JetByJetHists' + version.capitalize() + '_%i%i_tau%06ium_%s' % (nl, nb, tau, batch_tag)
            args = '-t mfvMovedTree%i%i --tau %i %s' % (nl, nb, tau, correction_args) 
