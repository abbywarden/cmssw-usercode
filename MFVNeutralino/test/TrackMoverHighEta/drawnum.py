...
def get_em(fn, scale=1., alpha=1-0.6827):
    ...
    for key in f.GetListOfKeys():
        ...
        for var in variables:
           if var in name:
             Isnametoplot = True
             break
        if not Isnametoplot or "which_pv" in name:
           continue
    ...
    for name in ltmp:
      if name.endswith('_num'):
          ...
          g = num  #FIXME den #histogram_divide(num, den, confint_params=(alpha,), use_effective=use_effective)
          g.SetTitle('')
          g.GetXaxis().SetTitle(num.GetXaxis().GetTitle())
          g.GetYaxis().SetTitle('efficiency')
          rat_name = name # FIXME name.replace('_num', '_den')
    ...
    for cutset, num_int, num_err, den_int, den_err in cutsets:
        ...
        g = num #FIXME den #histogram_divide(num, den, confint_params=(alpha,), use_effective=use_effective)
        rat_name = cutset + '_num' #FIXME '_den'
        d[rat_name] = g
        l.append(rat_name)
...
def comp(ex, fn1='data.root', fn2='ttbar.root', fn3='mergedqcd.root', fn4='dyjets.root', fn5 = 'wjets_amcatnlo.root', fn6 = 'qcdmu15.root', fn7 = 'signal.root', Isshort='dummy', whichlep='Muon'):
    ...
    for name in l:
        ...
        if not name.startswith('all_'): #FIXME ('nocuts_'): #or "nm" in name:
            continue
        if name.endswith('_num'): #FIXME('_den'): #or (not name.endswith('_num') and not name.endswith('_den')):
            print(name) 
            ...
            if name.endswith('_num'): #FIXME ('_den'):
                for g in tot:
