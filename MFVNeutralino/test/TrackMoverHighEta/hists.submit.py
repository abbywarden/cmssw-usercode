          ...
          if apply_correction:
            for tm in ["sim","dat",]: #"dat"] :
              ...
              if tm == "sim":
                ...
                w_fn_2d_move = "reweight_v2p5_higheta_move_sim_vetodr_tau%06ium_M%02i_%s_2D.root" % (tau, mg, year) 
              else:
                ...
                w_fn_2d_move = "reweight_v2p5_higheta_move_dat_vetodr_tau%06ium_M%02i_%s_2D.root" % (tau, mg, year) 
              batch_tag = "2Dmovedist3movedist2jetdrllpsumpcoarse60alletaCorrection"
              w_fn_2d_kin = "reweight_v2p5_alleta_kin_sim_vetodr_tau%06ium_M%02i_2D.root" % (tau, mg) 
              ...
              batch = 'TrackMover_StudyV2p5_HighEta_NoPreSelRelaxBSPNotwVetodR0p4JetByJetHists' + version.capitalize() + '_%i%i_tau%06ium_M%02i_%s' % (nl, nb, tau, mg, batch_tag)
            ...
            batch = 'TrackMover_StudyV2p5_HighEta_NoPreSelRelaxBSPNotwVetodR0p4JetByJetHists' + version.capitalize() + '_%i%i_tau%06ium_%s' % (nl, nb, tau, batch_tag)
            args = '-t mfvMovedTree%i%i --tau %i %s' % (nl, nb, tau, correction_args) 
